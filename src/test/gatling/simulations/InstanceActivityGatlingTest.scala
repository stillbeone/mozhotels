...
/**
 * Performance test for the InstanceActivity entity.
 */
class InstanceActivityGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the InstanceActivity entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all instanceActivities")
            .get("/api/instance-activities")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created instanceActivity")
                .get("${new_instanceActivity_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created instanceActivity")
            .delete("${new_instanceActivity_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
